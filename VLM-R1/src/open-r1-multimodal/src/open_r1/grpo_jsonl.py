...
class GRPOScriptArguments(ScriptArguments):
    ...
    point_expand_size: int = field(
        default=10,
        metadata={"help": "Size to expand point to bbox when only 2 coordinates are found"},
    )
...
def voting_reward(completions, **kwargs):
    print(len(completions), "completions length")
    contents = [completion[0]["content"] for completion in completions]
    print(len(contents), "contents length")
    rewards = []
    sampled_bboxes = []
    W, H = kwargs.get("image_size")[0]
    expand_size = kwargs.get("point_expand_size")[0]
    model = kwargs.get("model")[0]
    grid = torch.zeros((H, W), dtype=torch.int32)
    for content in contents:
        print("content:", content)
        bbox = extract_answer(content)
        if "intern" in model.lower():
            x1, y1, x2, y2 = int(bbox[0]*W)-expand_size, int(bbox[1]*H)-expand_size, int(bbox[2]*W)+expand_size, int(bbox[3]*H)+expand_size
        elif "tars" in model.lower():
            x1, y1, x2, y2 = int(bbox[0])-expand_size, int(bbox[1])-expand_size, int(bbox[2])+expand_size, int(bbox[3])+expand_size
        elif "atlas" in model.lower():
            x1, y1, x2, y2 = int(bbox[0]/1000*W), int(bbox[1]/1000*H), int(bbox[2]/1000*W), int(bbox[3]/1000*H)
        elif "uground" in model.lower():
            x1, y1, x2, y2 = int(bbox[0]/1000*W)-expand_size, int(bbox[1]/1000*H)-expand_size, int(bbox[2]/1000*W)+expand_size, int(bbox[3]/1000*H)+expand_size
        else:
            x1, y1, x2, y2 = int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])
        x1, y1 = max(0, x1), max(0, y1)
        x2, y2 = min(W, x2), min(H, y2)
        print(f"sample bbox: [{x1}, {y1}, {x2}, {y2}]")
        sampled_bboxes.append([x1, y1, x2, y2])
        grid[y1:y2, x1:x2] += 1

    max_voting = grid.max().item()
    for bbox in sampled_bboxes:
        region = grid[bbox[1]:bbox[3], bbox[0]:bbox[2]]
        region_w, region_h = region.shape
        sum_voting = region.sum().item()
        if region_w == 0 or region_h == 0:
            reward = 0
        else:
            reward = sum_voting / (max_voting * region_w * region_h)
        rewards.append(reward) 
        ...
        if os.getenv("DEBUG_MODE") == "true":
            ...
            image_path = kwargs.get("image_path")[0] if "image_path" in kwargs else None
            problem = kwargs.get("problem")[0]
            solution = kwargs.get("solution")[0]
            gt_bbox = [solution[0]* W, solution[1]*H, solution[2]*W, solution[3]*H]
            if reward <= 1.0:  # this condition can be changed for debug
                with open(log_path, "a", encoding='utf-8') as f:
                    f.write(f"------------- {current_time} Voting reward: {reward} -------------\n")
                    f.write(f"image_path: {image_path}\n")
                    f.write(f"problem: {problem}\n")
                    f.write(f"sampled_bbox: {bbox}\n")
...
QUESTION_TEMPLATE = (
    "You are a GUI analysis expert. Based on the screenshot, your task is to locate the UI element that best matches the instruction:\n"
    "'{instruction}'\n"
...
def get_vlm_module(model_name_or_path):
    if "qwen" in model_name_or_path.lower() or "ui-tars" in model_name_or_path.lower() or "atlas" in model_name_or_path.lower() or "uground" in model_name_or_path.lower():
...
def main(script_args, training_args, model_args):
    ...
    if "qwen" in model.lower():
        ...
        QUESTION_TEMPLATE = ("In this UI screenshot, what is the position of the element corresponding to the command '{instruction}' (with bbox)?")
    elif "tars" in model.lower():
        QUESTION_TEMPLATE = (
            "You are a GUI analysis expert. Based on the screenshot, your task is to identify the center point of the UI element that best matches the instruction:\n"
            "'{instruction}'\n"
            "You MUST output exactly one point coordinate in the format [x, y] strictly. Do not include any explanations, descriptions, or additional text.\n"
            "**Output format: [x, y]**"
        )
    elif "intern" in model.lower():
        SYSTEM_PROMPT = ("You are InternVL, a GUI agent. \n\nYou are given a task and a screenshot of the screen. You need to perform a series of pyautogui actions to complete the task.")
        QUESTION_TEMPLATE = ("\n<image>\n{instruction}")
    elif "uground" in model.lower():
        QUESTION_TEMPLATE = (
    ...
    print("question_prompt:", QUESTION_TEMPLATE)
    # Load the VLM module
    ...
    for data_file, image_folder, accu_reward_method in zip(data_files, image_folders, accu_reward_methods):
        with open(data_file, 'r') as f:
            for line in f:
                ...
                item['problem'] = item['instruction']
                item['solution'] = item['bbox']
    ...
    def make_conversation_from_jsonl(example):
        if 'image_path' in example and example['image_path'] is not None:
            ...
            if "intern" in example['model'].lower():
                return{
                    ...
                    'prompt': [
                        ...
                        },
                        {
                            'role': 'user',
                            'content': [
                                {"type": "text", "text": QUESTION_TEMPLATE.format(instruction=example['problem'])}
                            ]
                    }]
                }
            else:
    ...
    trainer = trainer_cls(
        ...
        point_expand_size=script_args.point_expand_size,
    )
